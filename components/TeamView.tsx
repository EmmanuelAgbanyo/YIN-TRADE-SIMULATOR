...
export const TeamView: React.FC<TeamViewProps> = ({ profile, orderHistory }) => {
    ...
    useEffect(() => {
        if (profile.teamId) {
            const allTeams = safeJsonParse<Team[]>('yin_trade_teams', []);
            const currentTeam = allTeams.find(t => t.id === profile.teamId);
            setTeam(currentTeam || null);

            if (currentTeam) {
                const allProfiles = safeJsonParse<UserProfile[]>('yin_trade_profiles', []);
                const teamMembers = allProfiles.filter(p => currentTeam.memberIds.includes(p.id));
                setMembers(teamMembers);

                const allInvites = safeJsonParse<TeamInvite[]>('yin_trade_invites', []);
